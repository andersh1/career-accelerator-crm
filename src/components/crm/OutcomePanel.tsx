    ...
    <div className="rounded-2xl border shadow-sm overflow-hidden" style={{ borderColor: "#e4e0d6", background: "white" }}>
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b" style={{ borderColor: "#e4e0d6", background: "#f8f6f1" }}>
        <div className="flex items-center gap-2">
          <Trophy size={14} style={{ color: "#0a6b64" }} />
      ...
      {!editing && (
        <div className="px-5 py-4">
          {!hasData ? (
            <p className="text-xs text-center py-3" style={{ color: "#8a938f" }}>
            ...
            <div className="space-y-2.5">
              ...
              {outcome.outcomeNotes && (
                <div className="flex items-start gap-2.5 pt-1">
                  <FileText size={13} className="mt-0.5 shrink-0" style={{ color: "#8a938f" }} />
      ...
      {editing && (
        <div className="px-5 py-4 space-y-3">
          {/* Status */}
          <div>
            <label className="text-xs font-semibold mb-1 block" style={{ color: "#8a938f" }}>Status</label>
            <div className="flex flex-wrap gap-2">
              {OUTCOME_STATUSES.map(s => (
                <button key={s.key}
                  onClick={() => setStatus(s.key)}
                  className={`text-xs font-semibold px-3 py-1 rounded-full border transition ${status === s.key ? s.color + " border-transparent" : "border-[#e4e0d6] text-slate-500 hover:border-[#0a6b64]"}`}>
                  {s.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs font-semibold mb-1 block" style={{ color: "#8a938f" }}>Company</label>
            ...
            <div>
              <label className="text-xs font-semibold mb-1 block" style={{ color: "#8a938f" }}>Role / Title</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs font-semibold mb-1 block" style={{ color: "#8a938f" }}>Salary (annual $)</label>
            ...
            <div>
              <label className="text-xs font-semibold mb-1 block" style={{ color: "#8a938f" }}>Start date</label>
