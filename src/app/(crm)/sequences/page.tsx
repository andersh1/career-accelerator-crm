  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-72 shrink-0 border-r border-[#e4e0d6] flex flex-col" style={{ background: "#f8f6f1" }}>
        <div className="px-4 py-4 border-b border-[#e4e0d6] flex items-center justify-between">
          <div>
            ...
            <p className="text-xs" style={{ color: "#8a938f" }}>{sequences.length} total</p>
          </div>
          <button onClick={() => setShowNew(v => !v)}
            className="p-1.5 rounded-lg text-white transition" style={{ background: "#0a6b64" }}>
            <Plus size={14} />
          </button>
        </div>
        ...
        {showNew && (
          <div className="px-4 py-3 border-b border-[#e4e0d6] bg-white space-y-2">
            <input autoFocus value={newName} onChange={e => setNewName(e.target.value)}
              ...
              className="w-full text-sm border border-[#e4e0d6] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0a6b64]" />
            <input value={newDesc} onChange={e => setNewDesc(e.target.value)}
              placeholder="Description (optional)"
              className="w-full text-sm border border-[#e4e0d6] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0a6b64]" />
            <div className="flex gap-2">
              <button onClick={create} disabled={creating || !newName.trim()}
                className="flex-1 text-xs font-semibold text-white py-1.5 rounded-lg disabled:opacity-50 transition" style={{ background: "#0a6b64" }}>
                {creating ? "Creating…" : "Create"}
              </button>
              <button onClick={() => setShowNew(false)}
                className="px-3 text-xs transition" style={{ color: "#8a938f" }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="py-12 px-4 text-center text-xs" style={{ color: "#8a938f" }}>
            ...
            <button key={seq.id} onClick={() => setSelected(seq)}
              className={`w-full flex items-center gap-3 px-4 py-3.5 border-b border-[#e4e0d6] text-left transition ${
                selected?.id === seq.id ? "border-l-2 border-l-[#0a6b64]" : "hover:bg-white"
              }`}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!selected ? (
          <div className="h-full flex items-center justify-center text-sm" style={{ color: "#8a938f" }}>
          ...
          <div className="max-w-2xl mx-auto space-y-5">
              ...
              {renamingSeq ? (
                <div className="flex-1 mr-4 space-y-2">
                  <input
                    ...
                    style={{ border: "1px solid #0a6b64", color: "#14211f" }}
                    placeholder="Sequence name"
                  />
                  <input
            ...
            {selected.steps.length === 0 && (
              <div className="border rounded-2xl p-4" style={{ background: "#edf5f4", borderColor: "#e4e0d6" }}>
                <p className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: "#0a6b64" }}>Quick-start templates</p>
                <div className="space-y-2">
                  {TEMPLATE_STEPS.map(t => (
                    <button key={t.label} onClick={() => applyTemplate(t.steps)}
                      className="w-full flex items-center gap-3 px-4 py-3 bg-white rounded-xl border text-left transition hover:border-[#0a6b64]"
                      style={{ borderColor: "#e4e0d6" }}>
                      <Mail size={14} className="shrink-0" style={{ color: "#0a6b64" }} />
                      <div>
            ...
            {selected.steps.length > 1 && (
              <div className="flex items-start gap-2.5 border rounded-xl px-4 py-3" style={{ background: "#edf5f4", borderColor: "#e4e0d6" }}>
                <span className="mt-0.5 shrink-0" style={{ color: "#0a6b64" }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                  </svg>
                </span>
                <p className="text-xs leading-relaxed" style={{ color: "#0a6b64" }}>
                ...
                <div key={step.id} className="card shadow-sm overflow-hidden">
                  {editStep?.id === step.id ? (
                    <div className="p-4 space-y-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold uppercase tracking-wide" style={{ color: "#8a938f" }}>Step {step.stepNumber}</span>
                        <input type="number" min={0}
                          value={editStep.delayDays}
                          onChange={e => setEditStep(s => s ? { ...s, delayDays: parseInt(e.target.value) || 0 } : s)}
                          className="w-16 text-xs border border-[#e4e0d6] rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#0a6b64] ml-auto"
                        />
                        <span className="text-xs" style={{ color: "#8a938f" }}>days delay</span>
                      </div>
                      <input value={editStep.subject}
                        onChange={e => setEditStep(s => s ? { ...s, subject: e.target.value } : s)}
                        placeholder="Subject line"
                        className="w-full text-sm border border-[#e4e0d6] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0a6b64]" />
                      <textarea value={editStep.body} rows={6}
                        ...
                        className="w-full text-sm border border-[#e4e0d6] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0a6b64] resize-none font-mono text-xs" />
                      <div className="flex gap-2">
                        <button onClick={saveStep} disabled={savingStep}
                          className="flex items-center gap-1.5 text-xs font-semibold text-white px-3 py-1.5 rounded-lg disabled:opacity-50 transition" style={{ background: "#0a6b64" }}>
                          {savingStep ? <Loader2 size={11} className="animate-spin" /> : <Check size={11} />} Save
                        </button>
                        <button onClick={() => setEditStep(null)}
                          className="text-xs px-3 py-1.5 transition" style={{ color: "#8a938f" }}>
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-4">
                      <div className="flex flex-col items-center gap-1 shrink-0">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "#edf5f4" }}>
                          <span className="text-[11px] font-bold" style={{ color: "#0a6b64" }}>{step.stepNumber}</span>
                        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[11px] font-bold uppercase tracking-wide" style={{ color: "#8a938f" }}>
                        ...
                        <p className="text-xs mt-0.5 line-clamp-2" style={{ color: "#8a938f" }} title={step.body}>{step.body}</p>
                      </div>
            ...
            {addingStep ? (
              <div className="card shadow-sm p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    <span className="text-xs" style={{ color: "#8a938f" }}>Delay:</span>
                    <input type="number" min={0} value={newStep.delayDays}
                      onChange={e => setNewStep(s => ({ ...s, delayDays: parseInt(e.target.value) || 0 }))}
                      className="w-16 text-xs border border-[#e4e0d6] rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#0a6b64]"
                    />
                    <span className="text-xs" style={{ color: "#8a938f" }}>days</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setAddingStep(false)} className="text-xs px-3 py-1.5 transition" style={{ color: "#8a938f" }}>
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <button onClick={() => setAddingStep(true)}
                className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold border-2 border-dashed border-[#e4e0d6] rounded-2xl hover:border-[#0a6b64] hover:bg-[#edf5f4] transition"
                style={{ color: "#8a938f" }}>
