  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6 animate-fade-up">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "#949598" }}>Send a one-off email to a filtered segment of leads. Enrolled and lost leads are excluded by default.</p>
      </div>

      {/* Filters */}
      <div className="card shadow-sm p-5 space-y-4">
        <h2 className="font-semibold text-sm" style={{ color: "#14211f" }}>Audience Filters</h2>
        <p className="text-xs" style={{ color: "#949598" }}>Leave filters empty to reach all active leads.</p>
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <select value={stage} onChange={e => { setStage(e.target.value); setPreview(null); setConfirmed(false); }}
              className="w-full border border-[#e4e0d6] rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#086c64]">
        ...
        <button onClick={checkRecipients} disabled={checking}
          className="flex items-center gap-2 text-sm font-semibold transition disabled:opacity-50" style={{ color: "#086c64" }}>
        ...
        {preview && (
          <div className="border rounded-xl p-3" style={{ background: "#edf5f4", borderColor: "#c5dedd" }}>
            <p className="text-sm font-bold" style={{ color: "#086c64" }}>{preview.count} lead{preview.count !== 1 ? "s" : ""} will receive this email</p>
            {preview.preview.length > 0 && (
              <p className="text-xs mt-1" style={{ color: "#086c64" }}>
      ...
      <div className="card shadow-sm p-5 space-y-4">
        ...
        <div>
          ...
          <input value={subject} onChange={e => setSubject(e.target.value)}
            className="w-full border border-[#e4e0d6] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#086c64]"
        ...
        <div>
          ...
          <textarea value={body} onChange={e => setBody(e.target.value)} rows={10}
            className="w-full border border-[#e4e0d6] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#086c64] resize-y font-mono"
        ...
        {result ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "#949598" }}>{result.sent} delivered{result.failed > 0 ? `, ${result.failed} failed` : ""}</p>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <button
              ...
              style={{ background: sending || !subject.trim() || !body.trim() || !confirmed ? "#e4e0d6" : "#086c64" }}
            >
